...
@WebServlet(name = "/LoginServlet", urlPatterns = { "/login" }, initParams = {
        ...
        @WebInitParam(name = "user", value = "root"), @WebInitParam(name = "password", value = "654321") })
public class LoginServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ResultSet resultSet = dbDao.queryUser("select password from user " + " where userName = ?", userName);
            if (resultSet.next()) {
